...
- Performance. I have not needed to do any performance optimizations yet. I should probably do some in the future.
  ...
  - The sprite rendering is extremely primative. There is no batching or optimization done at all. Each sprite is drawn individually with its own draw call. Spawning hundreds of bees drops the framerate (130ish -> 100ish), and it keeps decreasing as more bees are added. I'm unsure if this is directly because of the draw calls, it could be the box2d system running slower or the path finding system is hogging the tile system, which is needed for rendering. Either way, I need to optimise the rendering system.
- Occasionally the path finding breaks and the bees either stay in place or path find to (0,0). I have no idea why this happens.
