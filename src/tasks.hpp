...
namespace game
{
	...
	struct create_debris_params
	{
		...
		create_debris_params(float x_pos, float y_pos, float vel_x, float vel_y, float radius, uint8_t debri_tile_type, uint8_t perm_tile_type, uint8_t temp_tile_type, uint16_t millis_lifetime, float tile_temp) : x(x_pos), y(y_pos), v_x(vel_x), v_y(vel_y), r(radius), debri_tile_type(debri_tile_type), perm_tile_type(perm_tile_type), temp_tile_type(temp_tile_type), millis_lifetime(millis_lifetime), tile_temperature(tile_temp) {}
	};
